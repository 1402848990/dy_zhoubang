...
@app.route('/qrcode_url.php', methods=['GET', 'POST', 'OPTIONS'])
def get_user_info():
    sec_user_id = request.args.get('sec_user_id', '').strip()
    if not sec_user_id:
        sec_user_id = 'MS4wLjABAAAAKE9hcXbTu7QWueiSlhQc4fuU5l48cjFaCYnXtRPV9Fc'
    ...
    url = (
        ...
        f"&browser_language=zh-CN&browser_platform=Win32&browser_name=Chrome&browser_version=143.0.0.0"
        f"&browser_online=true&engine_name=Blink&engine_version=143.0.0.0&os_name=Windows&os_version=10"
        f"&device_memory=8&platform=PC&downlink=10&effective_type=4g&round_trip_time=150"
        f"&webid=7498378399590024719"
    )
    ...
    try:
        resp = requests.get(url, headers=headers, cookies=dict(item.split('=', 1) for item in COOKIE.split('; ') if '=' in item), timeout=30)
        data = resp.json()
        return jsonify(data)
